...
function Paragraph({ image, index, offset, factor, header, aspect, text }) {
  ...
  const left = !(index % 2)
  const color = index % 2 ? "#ed1a25" : "#fcd900"
  return (
  ...
  return (
    <>
      <Block factor={1} offset={0}>
        <Block factor={1.2}>
          <Text left size={w * 0.08} position={[-w / 3.2, 0.5, -1]} color="#ffffff">
            ACY
          </Text>
        </Block>
        <Block factor={1.0}>
          <Dom position={[-w / 3.2, -w * 0.08 + 0.25, -1]}>Ahmet Can YILMAZ{mobile ? <br /> : " "}Designer from ISTANBUL.</Dom>
        </Block>
      </Block>
      <Block factor={1.2} offset={5.7}>
        <MultilineText top left size={w * 0.15} lineHeight={w / 4} position={[-w / 3.5, 0, -1]} color="#fcd900" text={"A.\nC.\nY."} />
      </Block>
